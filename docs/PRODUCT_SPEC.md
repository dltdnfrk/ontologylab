...
- approved_by: hyunjun
  ...
  gate: product-identity-clarification
  checks: OntologyLab의 기존 근거→검증 지식 역할을 소유자 지시로 명문화하고 구현 감사 문서와 대조
```
...
> **2026-08-08에는 제품 기능을 바꾸지 않고**, OntologyLab의 제품 정체성을
> 명문화하고 현재 구현이 그 정의를 지키는지 감사했다.
...
OntologyLab의 원재료는 **이미 존재하는 외부 근거**다. 논문·기술문서·공식 레지스트리에서
주장과 관계를 추출하고, 원문 위치와 출처를 보존하고, 엔티티를 정규화·연결하고, 사람의
검증을 거쳐 재사용 가능한 지식으로 출고한다. 근거에 없는 사실이나 미래 결과를 생성하는
것은 제품 목표가 아니다.

| 항목 | OntologyLab |
| --- | --- |
| 출발점 | 이미 존재하는 문헌·데이터·공식 레지스트리 |
| 핵심 변환 | 근거 추출·검증·정규화·연결 |
| 결과물 | 출처와 승인 이력이 있는 검증 지식 팩 |

따라서 OntologyLab의 품질 기준은 생성 결과의 참신성이 아니라 **근거 커버리지, 출처
추적성, 추출 정확도, 검토 가능성, 그래프 연결성, 검색·질의 유용성**이다. LLM은 근거를
찾고 구조화하는 도구이며, 지식을 새로 발명하는 권한이나 최종 승인 권한을 갖지 않는다.
...
- 모듈 수준 상수·조회표가 파일이 **리터럴로 선언한** 값과 다른 값을 들고 있는 경우
  (한정된 보장이다. 묶이는 할당 형태와 묶이지 않는 잔여는 §7.1.1)
...
1. (`hostile-1`) 실행 비밀키(run secret)는 자식 프로그램 `__main__`의 전역으로 접근 가능하다.
   제품 코드에서 읽어 위조 보고서에 서명할 수 있다.
2. (`hostile-2`) 첫 부모 프로세스에서 비밀키 배출 이전에 실행되는 시작 코드·표준 라이브러리 가림.
   `python scripts/check_product_status.py`는 호출에 `-P`가 없으면 실제 모듈보다
   `scripts/subprocess.py`를 먼저 가져온다.
3. (`hostile-3`) `LIBRARY_ROOTS`에 있는, 해시되지 않은 설치 의존성(pytest, pluggy 등)의 변조.
...
#### 7.1.1 부분 보장 — 모듈 수준 데이터

위 목록의 데이터 항목은 **파일이 리터럴로 선언한 값에 한정된다**. 게이트가 값을 묶는
할당 형태는 다음이 전부다.

- 최상위 `Assign` / `AnnAssign` 중 `ast.literal_eval`이 그대로 평가하는 값
- 리터럴 인자 하나를 감싼 `frozenset(...)`, `set(...)`, `tuple(...)`, `list(...)`,
  `dict(...)`

**묶이지 않는 것(잔여, residual).** 값이 표현식에서 파생되면 파일만 읽어서는 알 수 없고,
게이트는 이를 검사하지 않는다. 해당 AST 노드 종류: `Call`, `Attribute`, `Name`, `BinOp`,
`Dict`, `Tuple`, `JoinedStr`.

**실무상 귀결.** 다른 모듈의 파생 상수를 런타임에 바꿔 쓰는 흔한 리팩터링 잔재는
탐지되지 않는다. 예: 어떤 모듈이 `import ontologylab.registry as r` 뒤에
`r._MAX_INFO_BYTES = 2 * 1024 * 1024`를 남겨 두면, `registry.py`가 선언한 값과 달라져도
게이트는 통과한다(파일 선언값이 `1024 * 1024`, 즉 `BinOp`이기 때문이다).

**분류: `residual-4-non-hostile`.** 이것은 §7.1의 세 가지 적대적 제외 사례
(`hostile-1`, `hostile-2`, `hostile-3`)와 **다른 종류의 항목**이며, 그 목록에 넣지
않는다. 저 셋은 이 체커를 겨냥해 작성된 코드를 두고 내린 결정이고, 이 잔여는 사고 모델
안에서 평범한 리팩터링 잔재에 해당하는 정적 분석의 한계다. 여기서 런타임 강제를
넓히지 않는다는 것도 함께 기록한다.

#### 7.1.2 부분 보장 — 함수 호출 상태

함수 기본값은 위치별로 묶인다. 소스가 결정하는 `__defaults__`의 존재 여부와 길이,
`__kwdefaults__`의 존재 여부와 키 집합은 항상 검사하며, 각 위치·키에서 정적으로 평가할
수 있는 리터럴과 중첩 컨테이너 값은 타입을 보존한 인코딩으로 검사한다. 하나의 함수에
리터럴 기본값과 이름 조회·호출 같은 비리터럴 기본값이 섞여 있어도 비리터럴 위치 하나가
리터럴 형제 위치의 검사를 없애지 않는다.

**묶이지 않는 보통의 잔여.** 비리터럴 기본값 표현식의 *값 자체*는 실행하지 않고 해당
위치만 명시적인 unbound 상태로 둔다. 따라서 그 위치에서 소스와 같은 형태의 다른 런타임
값으로 바뀌는 것은 탐지하지 못한다. 함수 closure cell 내용도 팩토리 인자와 실행 경로에
의존하므로 이 정적 동일성 검사에 포함하지 않는다. 이는 `hostile-1/2/3`과 별개인 평범한
정적 분석 한계다. 단, 비리터럴 위치라도 기본값 튜플의 존재·길이와 keyword-only 키는
...
- **2026-08-03 §7.1 데이터 보장 한정** — §7.1.1 신설. 모듈 수준 데이터 보장이 리터럴
  할당 형태에 한정된다는 사실과, 표현식에서 파생된 값이 묶이지 않는다는 잔여를 명시했다.
  계기: 최종 게이트 검증에서 `r._MAX_INFO_BYTES = 2 * 1024 * 1024` 형태의 평범한
  교차 모듈 리팩터링 잔재가 통과했는데, 당시 §7.1 문장은 한정 없이 "다른 값을 들고 있는
  경우"를 막는다고 적고 있었다. 코드의 잔여는 의도된 것이었으나 계약이 과대 진술이었다.
  검사를 약화하거나 digest를 갱신하지 않았고 런타임 강제도 넓히지 않았다. 수락기준
  표·상태·근거 경로는 그대로다.
