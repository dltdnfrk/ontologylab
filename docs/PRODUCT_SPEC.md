...
### D9. MCP 읽기 범위 — 2티어, 전부 읽기 전용 (2026-08-02 신규)

MCP 표면은 **모든 시스템 상태에 대해 읽기 전용**이며 쓰기 경로는 노출하지 않는다.
두 티어가 있다(`tests/test_mcp_two_tier.py`): **live 티어**는 현재 verified 상태를,
**pack 티어**는 불변 스냅샷을 제공한다. `pending_verified_count`는 live 티어의
자연스러운 위치에서 쿼리 시점 계산으로 노출되고, pack 티어는 verified-only·불변을
유지한다.
