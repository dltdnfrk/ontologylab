...
- **Selection.** `get_engine("api:<id>", ..., data_dir=...)` loads the provider from the given data dir and returns an `ApiEngine`; unknown → `EngineError`. Engine-name validation (`is_valid_engine_name` / the argparse `type=`) accepts a built-in **or** an `api:<slug>`, replacing the rigid `choices=ENGINE_NAMES`. The CLI `provider` subcommands (`list`/`add`/`remove`/`test`) and the `/api/providers` routes drive the registry; the dashboard's Engines tab exposes the same surface. **The default engine is `claude` everywhere** (`paths.DEFAULT_ENGINE`; CLI, settings, and every API request model follow it). `mock` is a dev/test stub, selectable only by name — a networked provider runs when explicitly selected *and* its key env var is set.
