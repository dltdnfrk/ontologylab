...
- **Connectors are deny-by-default (strict allowlist), on both connectors.** `connectors/allowlist.py` is a **positive** allowlist: a request is permitted only if it matches an allowlisted entry, and anything else is rejected with a clear error (never silently skipped). This is enforced at **both** ingest connectors, from day one, not a retrofit:
  ...
  - **`paper_api`** enforces the allowlist at the same point as web crawl, with two gates matched to where risk actually lives. **Sources are a closed positive list** — each name maps to exactly one fixed endpoint constant, keeping the network boundary enumerable. Five are keyless (`arxiv`, `crossref`, `openalex`, `semanticscholar`, `europepmc`); three are publisher APIs that carry a credential in a request header (`elsevier`, `springer`, `core`) and are queried only once that credential is connected. A key never appears in a URL: it would otherwise reach the offline-refusal message, `provenance.jsonl`, `status.json`'s `last_payload` and the job log, none of which know a URL might be a secret. **Queries are validated, not enumerated** (non-empty, length-bounded, no control chars, no embedded URL): a query is only a percent-encoded search term inside a fixed endpoint and cannot redirect the request, so the original five-phrase positive list was dropped (2026-07) as an unusable training wheel. **Redirects are a third gate** (2026-07-26): `check_paper_host` re-validates every hop against `PAPER_API_HOSTS` — derived from the endpoint constants, exact match, https only — and any header outside `_REDIRECT_SAFE_HEADERS` is dropped when the host changes. `urllib` forwards all other headers, an API key included, to a redirect target; only `requests`/`urllib3` strip credentials across an origin. **Full text is a planned extension, not a permanent non-goal** — reached through publisher *APIs* (fixed hosts, exact-match allowlist entries), never by crawling document pages, whose `doi.org → publisher → CDN` redirect chains cannot be enumerated in advance. Today only title+abstract are ingested.
