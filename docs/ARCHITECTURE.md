...
**Chunking.** Documents are split before prompting: **target ~3,000 tokens per chunk with ~150-token (5%) overlap**, measured with a cheap heuristic tokenizer (≈ 4 chars/token; no model-specific tokenizer dependency). (The 3,000 target comes from the measured 1,500/3,000 sweep — `docs/CHUNK-SWEEP-2026-08.md`: recall preserved at half the calls.) Overlap exists so an entity/relation straddling a chunk boundary is seen whole by at least one chunk; entity resolution (§5.5) then dedups the overlap-induced duplicate mentions.
...
- **Selection.** `get_engine("api:<id>", ..., data_dir=...)` loads the provider from the given data dir and returns an `ApiEngine`; unknown → `EngineError`. Engine-name validation (`is_valid_engine_name` / the argparse `type=`) accepts a built-in **or** an `api:<slug>`, replacing the rigid `choices=ENGINE_NAMES`. The CLI `provider` subcommands (`list`/`add`/`remove`/`test`) and the `/api/providers` routes drive the registry; the dashboard's Engines tab exposes the same surface. **Engine defaults are split by surface, deliberately.** `paths.DEFAULT_ENGINE` is `claude` and `resolve_engine(None)` falls back to it, so an omitted CLI `--engine` — and `CriticRunRequest` / `ChatMessage` — runs claude. The surfaces a freshly installed desktop app can reach on its own (`ExtractRequest`, the research-start request, and the persisted `Settings`) default to `OFFLINE_LAUNCH_POLICY` instead — `engine="mock"`, `model=None` — because an omitted engine must never spawn a CLI or call a provider on someone's first run. So `mock` is not only a dev/test stub: on those paths it is the packaged offline default, and a live engine is an explicit selection. Translation refuses the offline default by validator (`translation requires an explicitly selected live engine`) rather than answering with the stub — and a networked provider runs when explicitly selected *and* its key env var is set.
...
**[Reconciled — as shipped.]** The table above is the original per-route design sketch. The shipped dashboard is a **single-page tabbed SPA** (`web/index.html` + one `web/app.js`), not one route per screen, and job progress is **pushed over SSE** (`GET /api/jobs/stream`, change-driven snapshots off a registry condition variable) with `GET /api/jobs` polling kept as the automatic fallback when EventSource is unavailable — the ROADMAP's "optional later upgrade" shipped 2026-07-23. Shipped tabs: **Review** (HITL queue + critic triage order + per-entity review panel, W8/W11), **Merge** (entity-merge candidate review, W7), **Sources**, **Jobs**, **Packs** (build + `.mcpb` bundle + pack diff), **MCP**, **Engines**, **Settings**. Two design-table promises were initially reconciled rather than built as separate pages; the **Graph Browser** later shipped (2026-07-23) as a **그래프 tab** — a read-only free-navigation SVG force-layout view (`GET /api/graph` overview + `GET /api/graph/neighbors/{id}` expansion, both thin wrappers over `graph_query`/`traverse_relations`), with status/type filters, highlight search, pan/zoom/drag, and a per-node card that jumps into the W11 entity review panel; per the HITL invariant the browser never mutates — approve/reject stay in Review. Per-entity inspection also remains available in the W11 panel, the CLI (`ontologylab entity`, `graph_query`), and any MCP client. The Packs row's "manifest diff vs. previous version" ships as W14 pack-diff (CLI `pack-diff` + `GET /api/packs/{a}/diff/{b}` + a Packs-tab panel). **[UX pass, 2026-07]** The SPA was later reorganized workflow-first with a Korean UI: a **Home** tab (pipeline stepper ① collect → ② extract → ③ review → ④ pack → ⑤ connect, live counts, next-action recommendation), primary tabs renumbered in pipeline order with auxiliary tabs (Merge/Communities/Engines/Settings) grouped after a separator, per-tab intro copy, and cross-tab jump links (`data-goto`). No API or element-id changes — cosmetic/navigation layer only.
...
**Algorithm tiers (2026-07).** Each core algorithm now has a literature-standard implementation with a stdlib fallback, and the choice is a property of the install, never of the data: communities are partitioned by **Leiden** (`ontologylab[graph]`; Traag et al. 2019 — connected communities guaranteed, which label propagation demonstrably is not: it collapses two bridged 5-cliques into one community) falling back to the original deterministic label propagation; hybrid search adds an optional **cross-encoder reranking stage** over the RRF shortlist (`auto` loads only a locally cached model — `HF_HUB_OFFLINE` during load, so a query can never be the thing that opens a network connection; `ontologylab rerank-download` is the one sanctioned egress); the merge scan generates candidate pairs by **character-3-gram blocking plus embedding nearest neighbours** instead of the full O(n²) cross product (`pairs_possible` vs `pairs_checked` in the scan stats keeps the reduction measurable); extraction quality is measured by `ontologylab eval` against a hand-written gold file (triple-level P/R/F1, exit-code CI gate); and the review queue's own approve/reject history feeds a **split-conformal triage line** (`GET /api/review/triage`) with a finite-sample guarantee — P(critic score of a rejection-worthy item > τ) ≤ α — that orders the queue and never approves anything.

Three statistical refinements sit on top (2026-07, round 2): `eval` reports a **percentile-bootstrap 95% CI** next to every F1, because at hand-written gold-set sizes a three-point F1 difference is routinely noise and "improved" should mean non-overlapping intervals; extractor confidences are audited against review outcomes with **ECE** and recalibrated by stdlib **isotonic regression (PAVA, ties pooled first)** at `GET /api/review/calibration` — read-only, stored confidences are provenance and never rewritten; and `detect_communities` accepts `algorithm="leiden-cpm"` with an explicit resolution γ, because modularity provably cannot resolve communities below ~√(L/2) internal edges (Fortunato & Barthélemy, PNAS 2007) — the test suite reproduces that limit on a ring of 20 triangles (modularity finds 8, CPM finds 20). CPM has deliberately no default γ: γ→0 degenerates to one giant community, and a silently-chosen constant would be a resolution limit with extra steps.
...
**What the job log keeps about you.** `jobs/<job_id>/provenance.jsonl` is append-only and nothing prunes it. Collect and research runs record their inputs there — the search topic, the URLs, the file paths — and `status.json` mirrors the same payload as `last_payload`. `Settings.cost_summary()` re-reads every one of those files, so a topic entered once stays on the machine and in every backup of it. This is intended: the audit trail is the point, and a run you cannot reconstruct is a run you cannot trust. What is bounded is the size, not the retention — `connectors.allowlist.loggable_collect_inputs` clips each recorded value at `MAX_PAPER_QUERY_LEN` and each list at `MAX_LOGGED_VALUES`, because inputs are logged *before* the gates run and an input headed for rejection would otherwise be archived in full. A topic string is never a path component: job directories are named for the stage (`research-<ts>`), and `paths.new_job_dir` rejects anything that is not a bare stage token.
...
- **Connectors are deny-by-default (strict allowlist), on both connectors.** `connectors/allowlist.py` is a **positive** allowlist: a request is permitted only if it matches an allowlisted entry, and anything else is rejected with a clear error (never silently skipped). This is enforced at **both** ingest connectors, from day one, not a retrofit:
  ...
  - **`paper_api`** enforces the allowlist at the same point as web crawl, with two gates matched to where risk actually lives. **Sources are a closed positive list** — each name maps to exactly one fixed endpoint constant, keeping the network boundary enumerable. Twelve sources are implemented (2026-08): eight keyless (`arxiv`, `crossref`, `openalex`, `semanticscholar`, `europepmc`, `biorxiv`, `pubmed`, `clinicaltrials`), three publisher APIs that carry a credential in a request header (`elsevier`, `springer`, `core`) and are queried only once that credential is connected, and `searxng` against an operator-run private instance. A key never appears in a URL: it would otherwise reach the offline-refusal message, `provenance.jsonl`, `status.json`'s `last_payload` and the job log, none of which know a URL might be a secret. **Queries are validated, not enumerated** (non-empty, length-bounded, no control chars, no embedded URL): a query is only a percent-encoded search term inside a fixed endpoint and cannot redirect the request, so the original five-phrase positive list was dropped (2026-07) as an unusable training wheel. **Redirects are a third gate** (2026-07-26): `check_paper_host` re-validates every hop against `PAPER_API_HOSTS` — derived from the endpoint constants, exact match, https only — and any header outside `_REDIRECT_SAFE_HEADERS` is dropped when the host changes. `urllib` forwards all other headers, an API key included, to a redirect target; only `requests`/`urllib3` strip credentials across an origin. **Full text** is ingested where open access provides it: `connectors/fulltext.py` rewrites a paper's body from the Europe PMC OA JATS (fixed allowlisted host) — never by crawling document pages, whose `doi.org → publisher → CDN` redirect chains cannot be enumerated in advance.
...
# Paper sources: closed positive list — each maps to one fixed endpoint
# (see connectors/paper_api.py). Keyless unless noted.
PAPER_API_SOURCES = {
    "arxiv", "crossref", "openalex", "semanticscholar", "europepmc",
    "biorxiv", "pubmed", "clinicaltrials",
    "elsevier", "springer", "core",   # keyed publisher APIs
    "searxng",                        # operator-run private instance
}
