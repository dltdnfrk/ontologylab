...
const alertVariants = cva("relative w-full rounded-lg border px-4 py-3 text-sm [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground [&>svg~*]:pl-7", {
  variants: {
    variant: {
      ...
      destructive: "border-destructive/50 bg-destructive-soft text-destructive [&>svg]:text-destructive",
      // 판정색은 옅은 면 + 같은 계열 글자로만 쓴다 (DESIGN.md 2장).
      success: "border-ok/40 bg-ok-soft text-ok-text [&>svg]:text-ok-text",
      warning: "border-warn/40 bg-warn-soft text-warn-text [&>svg]:text-warn-text",
    },
  },
