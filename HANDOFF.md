...
| W12 빌드타임 커뮤니티 (결정적 label propagation, extractive/LLM 요약, MCP `get_communities` 10번째 도구) | ✅ (`34f829a`) |
...
| **공개 전환 + CI + README** (2026-07-23): repo PUBLIC 전환(시크릿 스캔 클린, vault 경로 제거 `0399fb2`) → GitHub Actions CI(`.github/workflows/ci.yml` — pytest py3.11/3.12 + `node --check`, pip 캐시, 오프라인 스위트라 시크릿 불요) 첫 런에서 subprocess E2E의 `.venv/bin/python` 하드코딩 발견 → `sys.executable` 폴백(`9def04e`)으로 **CI 그린**. README 공개용 전면 정비(HITL 피치·스크린샷 3장·CI 뱃지·MCP 도구 10종 정정). LICENSE는 미정(사용자 결정 대기) | ✅ |
| **Graph Browser** (2026-07-23): 그래프 탭 — 의존성 0 vanilla SVG force-layout(반발/스프링/중력, rAF 쿨링), 팬·줌·드래그·클릭 선택·더블클릭 이웃 확장·강조 검색·상태(제안/검증)·타입 필터, 노드색은 `--chart-*` 토큰. 읽기 전용(HITL — 결정은 검토 탭 점프 `loadEntityPanel`). API `GET /api/graph`(graph_query 랩) + `GET /api/graph/neighbors/{id}`(traverse_relations 랩), 테스트 5종. 부수 수정: `graph_query` 엣지 IN절을 json_each 바인딩으로 교체(sqlite<3.32 host-param 999 한도, limit 500에서 초과하던 P2) | ✅ |
