...
def _open_copy(path: Path) -> KGStore:
    return KGStore.open(path)


def _zero(conn: sqlite3.Connection) -> None:
    record_drift_observation(
        conn, DriftObservation(generation=1, high_water=7, drift=0),
    )
...
def _walk(
    ...
    current = read_cutover_state(conn).phase
    for phase in CutoverPhase:
        ...
        if phase is CutoverPhase.DRAINED:
            _zero(conn)
            _zero(conn)
        advance_cutover(conn, phase, gates)
        if phase is target:
            return
...
def test_happy_path_reaches_authority_flipped_on_backup_copy(tmp_path: Path) -> None:
    ...
    store = _open_copy(copied)
    try:
        # Given a disposable backup-API copy
        bind = _bind(store.conn)
        # When the linear rehearsal runs to authority_flipped
        installed = install_cutover(store.conn, bind)
        assert installed.phase is CutoverPhase.EXPAND
        _walk(store.conn, CutoverPhase.FULL_V2)
        before = canonical_data_hash(store.conn)
        advance_cutover(
            store.conn, CutoverPhase.AUTHORITY_FLIPPED, CutoverChecks(pack_verified=True),
        )
        state = read_cutover_state(store.conn)
        # Then phase/receipts are exact and the flip is data-neutral
        ...
        assert state.backup_receipt == bind.backup_receipt
        kinds = [(row.kind, row.phase) for row in read_cutover_receipts(store.conn)]
        assert kinds == [
            ("transition", "expand"),
        ...
        assert canonical_data_hash(store.conn) == before
...
def test_one_zero_drift_cannot_drain(tmp_path: Path) -> None:
    ...
    try:
        ...
        _walk(store.conn, CutoverPhase.CATCH_UP)
        _zero(store.conn)
        with pytest.raises(CutoverRefused) as raised:
            advance_cutover(store.conn, CutoverPhase.DRAINED, CutoverChecks())
        assert raised.value.code is CutoverCode.DRIFT_REQUIRED
        assert read_cutover_state(store.conn).phase is CutoverPhase.CATCH_UP
    finally:
        store.close()


def test_nonzero_drift_resets_and_cannot_drain(tmp_path: Path) -> None:
    _source, copied, _hash = _snapshot_pair(tmp_path)
    store = _open_copy(copied)
    try:
        install_cutover(store.conn, _bind(store.conn))
        ...
        _zero(store.conn)
        record_drift_observation(
            store.conn, DriftObservation(generation=1, high_water=7, drift=1),
        )
        _zero(store.conn)
        with pytest.raises(CutoverRefused) as raised:
            advance_cutover(store.conn, CutoverPhase.DRAINED, CutoverChecks())
        assert raised.value.code is CutoverCode.DRIFT_REQUIRED
        assert read_cutover_state(store.conn).zero_drift_streak == 1
    finally:
        store.close()
...
def test_generation_mismatch_resets_and_cannot_drain(tmp_path: Path) -> None:
    ...
    try:
        ...
        _zero(store.conn)
        record_drift_observation(
            store.conn, DriftObservation(generation=2, high_water=7, drift=0),
        )
        _zero(store.conn)
        with pytest.raises(CutoverRefused) as raised:
            advance_cutover(store.conn, CutoverPhase.DRAINED, CutoverChecks())
...
def test_high_water_mismatch_resets_and_cannot_drain(tmp_path: Path) -> None:
    ...
    try:
        ...
        _zero(store.conn)
        record_drift_observation(
            store.conn, DriftObservation(generation=1, high_water=8, drift=0),
        )
        _zero(store.conn)
        with pytest.raises(CutoverRefused) as raised:
            advance_cutover(store.conn, CutoverPhase.DRAINED, CutoverChecks())
...
def test_live_writers_block_fence(tmp_path: Path) -> None:
    ...
    try:
        install_cutover(store.conn, _bind(store.conn))
        _walk(store.conn, CutoverPhase.DRAINED)
        with pytest.raises(CutoverRefused) as raised:
            advance_cutover(
                store.conn, CutoverPhase.FENCED, CutoverChecks(old_writer_count=1),
            )
        assert raised.value.code is CutoverCode.WRITERS_LIVE
        assert read_cutover_state(store.conn).phase is CutoverPhase.DRAINED
    finally:
        store.close()
...
def test_fenced_apply_v2_authority_mutation_is_refused(tmp_path: Path) -> None:
    ...
    try:
        bind = _bind(store.conn)
        install_cutover(store.conn, bind)
        _walk(store.conn, CutoverPhase.FENCED)
...
def test_integrity_blocker_refuses_constraints(
    ...
    try:
        ...
        with pytest.raises(CutoverRefused) as raised:
            advance_cutover(store.conn, CutoverPhase.CONSTRAINTS_REBUILT, blocked)
        assert raised.value.code is code
        assert read_cutover_state(store.conn).phase is CutoverPhase.FENCED
    finally:
        store.close()
...
def test_open_outbox_blocks_full_v2(tmp_path: Path) -> None:
    ...
    try:
        install_cutover(store.conn, _bind(store.conn))
        _walk(store.conn, CutoverPhase.CONSTRAINTS_REBUILT)
        with pytest.raises(CutoverRefused) as raised:
            advance_cutover(
                store.conn,
...
def test_unverified_pack_blocks_full_v2(tmp_path: Path) -> None:
    ...
    try:
        ...
        with pytest.raises(CutoverRefused) as raised:
            advance_cutover(
                store.conn, CutoverPhase.FULL_V2, CutoverChecks(pack_verified=False),
            )
...
def test_source_db_bytes_unchanged_after_rehearsal(tmp_path: Path) -> None:
    ...
    try:
        install_cutover(store.conn, _bind(store.conn))
        _walk(store.conn, CutoverPhase.AUTHORITY_FLIPPED)
    finally:
        store.close()
    assert _file_hash(source) == source_hash
    assert read_ledger(sqlite3.connect(f"file:{source}?mode=ro", uri=True)) == ()
...
def test_authority_flip_is_data_neutral(tmp_path: Path) -> None:
    ...
    try:
        ...
        _walk(store.conn, CutoverPhase.FULL_V2)
        before = canonical_data_hash(store.conn)
        docs = list(store.conn.execute("SELECT id, content_hash FROM documents"))
        advance_cutover(
            store.conn, CutoverPhase.AUTHORITY_FLIPPED, CutoverChecks(pack_verified=True),
        )
        assert canonical_data_hash(store.conn) == before
        assert list(store.conn.execute("SELECT id, content_hash FROM documents")) == docs
        assert all(row.phase != "post_cutover_write" for row in read_ledger(store.conn))
    finally:
        store.close()
...
def test_expand_zero_observations_are_refused(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert all(row.kind != "observation" for row in read_cutover_receipts(store.conn))
    finally:
        store.close()
...
def test_stale_drift_after_drain_blocks_later_phases(
    tmp_path: Path, start: CutoverPhase, blocked: CutoverPhase,
) -> None:
    _source, copied, _hash = _snapshot_pair(tmp_path)
    store = _open_copy(copied)
    try:
        install_cutover(store.conn, _bind(store.conn))
        _walk(store.conn, start)
        record_drift_observation(
            store.conn, DriftObservation(generation=1, high_water=7, drift=1),
        )
        assert read_cutover_state(store.conn).zero_drift_streak == 0
        with pytest.raises(CutoverRefused) as raised:
            advance_cutover(
                store.conn, blocked, CutoverChecks(pack_verified=True),
            )
        assert raised.value.code is CutoverCode.DRIFT_REQUIRED
        assert read_cutover_state(store.conn).phase is start
    finally:
        store.close()
