...
class CutoverChecks:
    ...
    open_outbox: int = 0; pack_verified: bool = False


@dataclass(frozen=True, slots=True)
class CutoverState:
    phase: CutoverPhase; generation: int; high_water: int
    source_fingerprint: str; backup_receipt: str; zero_drift_streak: int
...
def read_cutover_state(conn: sqlite3.Connection) -> CutoverState:
    ...
    row = conn.execute(
        ...
        "backup_receipt, zero_drift_streak FROM cutover_state WHERE id = 1"
    ).fetchone()
    if row is None:
        _refuse(CutoverCode.EMPTY_BIND, "not_installed")
    return CutoverState(
        CutoverPhase(str(row[0])), int(row[1]), int(row[2]), str(row[3]), str(row[4]), int(row[5]),
    )
...
def _persist(
    ...
    conn.execute(
        "UPDATE cutover_state SET phase = ?, zero_drift_streak = ? WHERE id = 1",
        (state.phase.value, state.zero_drift_streak),
    )
...
def install_cutover(conn: sqlite3.Connection, bind: CutoverBind) -> CutoverState:
    ...
    def _write() -> None:
        conn.execute(
            ...
            "zero_drift_streak INTEGER NOT NULL)")
        conn.execute(
        ...
        conn.execute(
            ...
            "source_fingerprint, backup_receipt, zero_drift_streak) "
            "VALUES (1, ?, ?, ?, ?, ?, 0)",
...
def record_drift_observation(
    ...
    *,
    ...
    """Count a zero only at the bound generation and high-water mark."""
    current = read_cutover_state(conn)
    matched = (
        observation.generation == current.generation
        and observation.high_water == current.high_water and observation.drift == 0
    )
    match current.phase:
        case CutoverPhase.CATCH_UP:
            streak = current.zero_drift_streak + 1 if matched else 0
        case (CutoverPhase.DRAINED | CutoverPhase.FENCED
              | CutoverPhase.CONSTRAINTS_REBUILT | CutoverPhase.FULL_V2
              | CutoverPhase.AUTHORITY_FLIPPED):
            if matched:
                _refuse(CutoverCode.WRONG_PHASE, current.phase.value)
            streak = 0
    ...
    nxt = CutoverState(
        current.phase, current.generation, current.high_water,
        current.source_fingerprint, current.backup_receipt, streak,
    )

    def _write() -> None:
        _persist(
            conn, nxt,
            ("observation", current.phase.value, "zero" if matched else "reset"),
            failpoint,
        )
...
def advance_cutover(
    ...
    *,
    ...
    nxt = CutoverState(
        ...
        current.backup_receipt, current.zero_drift_streak,
    )
    ...
    def _write() -> None:
        ...
        if target is CutoverPhase.AUTHORITY_FLIPPED and canonical_data_hash(conn) != before:
